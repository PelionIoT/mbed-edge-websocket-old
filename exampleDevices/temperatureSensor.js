...
EdgeJsonRpcExample.prototype._createDeviceParams = function(temperatureValue, displayMode) {
    ...
    let mode = Buffer.from(displayMode);
    mode = mode.toString('base64');

    // An IPSO/LwM2M temperature sensor and set point sensor (thermostat)
    params = {
        deviceId: 'TemperatureSensor-1',
        objects: [{
            objectId: 3303,
            objectInstances: [{
                objectInstanceId: 0,
...
EdgeJsonRpcExample.prototype.unregisterExampleDevice = async function() {
    ...
    return new Promise((resolve, reject) => {
        ...
        self.client.send('device_unregister', {deviceId: 'TemperatureSensor-1'},
            function(error, response) {
                clearTimeout(timeout);
                if (!error) {
                    resolve(response);
...
EdgeJsonRpcExample.prototype.updateExampleDeviceResources = async function() {
    ...
    return new Promise((resolve, reject) => {
        ...
        params = self._createDeviceParams(37 /* temp */,
                                          'fahrenheit' /* mode */);
